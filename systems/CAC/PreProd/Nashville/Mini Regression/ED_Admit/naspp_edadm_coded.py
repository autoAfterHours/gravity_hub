...
# ==========================================================================================================================================  
        ...
        if not iv_start_time:
            iv_start_time = prompt_value("Enter Date & Time for IV Start")

        if not iv_stop_time:
            iv_stop_time = prompt_value("Enter Date & Time for IV Stop")
...
# Add Provider Episode Navigation (Right Click Code)
        try:
            add_dr = page1.locator("#crsOuterIframe").content_frame.get_by_text(
                "</body> </html>").content_frame.get_by_text("Thoracoscopy, surgical; with"
                )
            if add_dr.is_visible():
                page1.locator("#crsOuterIframe").content_frame.get_by_text(
                    "</body> </html>").content_frame.get_by_text("Thoracoscopy, surgical; with").click(button="right"
                )
            else:
                raise Exception()
        except:
...
# ==================================================================
        ...
        page1.wait_for_timeout(1000)
        page1.locator("#crsOuterIframe").content_frame.get_by_text(
            "</body> </html>").content_frame.get_by_role("menuitem", name="Add Date/Physician/Episode").click()
        page1.wait_for_timeout(1000)
