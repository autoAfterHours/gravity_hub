...
# ========================================================================================================================================== 
        ...
        # Move Account from Hold to Ready
            section_break("Move Account to Ready")
            step("Update Account", "Updating Account to Ready Status")
            ...
            page1.wait_for_timeout(2000)
            page1.wait_for_load_state("domcontentloaded")
            logger.info("Selecting Ready Option")
            page1.get_by_text("Ready").click()
            logger.info("Account Updated to Ready")
            screenshot(page1, "account_updated_to_ready")
            end_step()

        # Add Diagnosis Code (I10)
            section_break("Add Diagnosis Code - I10")
            step("Add DX Code - I10", "Adding Diagnois Code (I10)")
            page1.wait_for_timeout(2000)
            page1.wait_for_load_state("domcontentloaded")
            page1.locator(".c_position_relative > div > .c_full_height > iframe").first.content_frame.get_by_text("</body> </html>").content_frame.get_by_role("button", name="Add Diagnosis").click()
            page1.wait_for_timeout(2000)
            page1.wait_for_load_state("domcontentloaded")
            ...
            page1.wait_for_load_state("domcontentloaded")
            page1.locator(".c_position_relative > div > .c_full_height > iframe").first.content_frame.get_by_text("</body> </html>").content_frame.get_by_role("textbox", name="Enter Keyword or Code:").fill("I10")
            page1.wait_for_timeout(500)
            page1.wait_for_load_state("domcontentloaded")            
            page1.locator(".c_position_relative > div > .c_full_height > iframe").first.content_frame.get_by_text("</body> </html>").content_frame.get_by_role("textbox", name="Enter Keyword or Code:").click()
            page1.wait_for_timeout(500)
